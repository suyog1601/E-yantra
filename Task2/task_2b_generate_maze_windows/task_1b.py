...
def weight(points, img):  # points r the 2d numpy aarray containging co ordinates of point
	...
	for _ in range(-10, 10):
		...
		if (a == 0 and j1 >= 0 and j1 < 15) or (
				a == 0 and ((j1 + j2) // 2 + _ > 0 and (j1 + j2) // 2 + _ < height) and (
				img[(j1 + j2) // 2 + _, (i1 + i2) // 2] == 0)):
			Ans = Ans + 2  # Adding weight         2         as it is closed at        up         side
			a = 1  # so this 2 should not add again
			ans.append(2)

		if (b == 0 and i1 >= 0 and i1 < 15) or (
				b == 0 and ((i1 + i3) // 2 + _ > 0 and (i1 + i3) // 2 + _ < length) and (
				img[(j1 + j3) // 2, (i1 + i3) // 2 + _] == 0)):
			Ans = Ans + 1  # Adding weight        1         as it is closed at          left        side
			b = 1
			ans.append(1)
			# cv.circle(img1,(  (i1+i3)//2 + _ , (j1+j3)//2      ), 2 , (0,0,255), -1)

		if (c == 0 and j3 <= height and j3 >= height - 10) or (
				c == 0 and ((j3 + j4) // 2 + _ > 0 and (j3 + j4) // 2 + _ < height) and (
				img[(j3 + j4) // 2 + _, (i3 + i4) // 2] == 0)):
			Ans = Ans + 8  # Adding weight          8           as it is closed at       down       side
			c = 1
			ans.append(8)
			# cv.circle(img1,(  (i3+i4)//2 , (j3+j4)//2 + _     ), 2 , (0,0,255), -1)

		if (d == 0 and i4 <= length and i4 >= length - 10) or (
				d == 0 and ((i2 + i4) // 2 + _ > 0 and ((i2 + i4) // 2 + _ < length)) and (
				img[(j2 + j4) // 2, (i2 + i4) // 2 + _] == 0)):
...
def applyPerspectiveTransform(input_img):
	...
	##############	ADD YOUR CODE HERE	##############
	img = cv2.resize(input_img, (500,500))
	imggray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
	edged = cv2.Canny(imggray, 30, 200)
	contour, heir = cv2.findContours(edged,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
	biggest = np.array([])
	max_area = 0
	for i in contour:
		...
		if area > 50:
			...
			if area > max_area and len(approx):
				biggest = approx
				max_area = area
