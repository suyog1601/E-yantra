...
def get_vision_sensor_image():
	...
	##############	ADD YOUR CODE HERE	##############
	return_code,sensorHandle = sim.simxGetObjectHandle(client_id,'Vision_sensor',sim.simx_opmode_blocking)
	return_code,image_resolution,vision_sensor_image = sim.simxGetVisionSensorImage(client_id,sensorHandle,0,sim.simx_opmode_oneshot_wait)
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	vision_sensor_image = cv2.flip(vision_sensor_image , 0) 
	vision_sensor_image = cv2.cvtColor(vision_sensor_image, cv2.COLOR_BGR2RGB )
	transformed_image = vision_sensor_image
...
def send_data(maze_array):
	...
	for i in maze_array :
		for j in i :
			wtf , aaaa , bbbb , cccc, dddd  = sim.simxCallScriptFunction( client_id ,  'Base' ,  6  ,  'receiveData' , [j] , [] ,  [str(count)] ,   bytearray(3)   ,     sim.simx_opmode_blocking  )
			count = count + 1 
	return_code = wtf 
	##################################################
